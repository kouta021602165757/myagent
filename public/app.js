...
function _showOnboarding(){
  ...
  document.getElementById('onbOverlay').classList.add('on');
}
function _closeOnboarding(){
  document.getElementById('onbOverlay').classList.remove('on');
}
function _renderOnboardingStep(step, state){
  const card = document.getElementById('onbCard');
  if(!card) return;
  if(step === 1){
    card.innerHTML = `
      <div class="onb-hd">
        <div class="onb-step">Step 1 / 3 — Welcome</div>
        <div class="onb-title">${isJa ? 'AI に何をやってもらいたい?' : 'What do you want AI to do for you?'}</div>
      </div>
      <div class="onb-body">
        <p class="onb-prompt">${isJa ? '1 文で OK。「AI が設計した専用エージェント」が 30 秒で立ち上がります。' : 'One sentence is enough. AI will design a custom agent for you in ~30 seconds.'}</p>
        <textarea id="onbGoal" class="onb-input" placeholder="${isJa ? '例: Instagram のアカウントを運営する / Shopify で商品を売る / 顧客サポートのメールを返す' : 'e.g. Run my Instagram / Sell products on Shopify / Reply to customer support emails'}"></textarea>
        <div class="onb-chips" id="onbChips"></div>
        <button class="onb-cta" id="onbGo" disabled>${isJa ? '▶ エージェントを設計する' : '▶ Design my agent'}</button>
        <button class="onb-skip" onclick="_skipOnboarding()">${isJa ? '今はスキップ' : 'Skip for now'}</button>
      </div>
    `;
    const examples = isJa
      ? ['Instagram を運営','Shopify で販売','顧客サポート','SEO 記事を書く','データを集計']
      : ['Run my Instagram','Sell on Shopify','Customer support','Write SEO articles','Crunch data'];
    const chipsEl = document.getElementById('onbChips');
    chipsEl.innerHTML = examples.map(e => `<button type="button" class="onb-chip" data-ex="${esc(e)}">${esc(e)}</button>`).join('');
    chipsEl.querySelectorAll('.onb-chip').forEach(c => c.onclick = () => {
      document.getElementById('onbGoal').value = c.dataset.ex;
      document.getElementById('onbGoal').dispatchEvent(new Event('input'));
    });
    const ta = document.getElementById('onbGoal');
    const btn = document.getElementById('onbGo');
    ta.oninput = () => { btn.disabled = ta.value.trim().length < 6; };
    btn.onclick = () => {
      const goal = ta.value.trim();
      if(goal.length < 6) return;
      _renderOnboardingStep(2, { goal });
      _runOnboardingGen(goal);
    };
    setTimeout(() => ta.focus(), 80);
  } else if(step === 2){
    card.innerHTML = `
      <div class="onb-hd">
        <div class="onb-step">Step 2 / 3 — ${isJa ? '設計中' : 'Designing'}</div>
        <div class="onb-title">${isJa ? 'AI があなたのエージェントを設計しています…' : 'AI is designing your agent…'}</div>
      </div>
      <div class="onb-body">
        <div class="onb-loading">
          <div class="ring"></div>
          <div class="lbl">${isJa ? '〜 30 秒' : '~ 30 seconds'}</div>
          <div class="sub">${isJa ? '目標を分析 → 役割と persona を生成 → 完成' : 'Analyzing goal → generating role + persona → done'}</div>
        </div>
      </div>
    `;
  } else if(step === 3){
    const a = state.agent || {};
    const skills = (a.skills || []).map(s => `<span>${esc(s)}</span>`).join('');
    card.innerHTML = `
      <div class="onb-hd">
        <div class="onb-step">Step 3 / 3 — ${isJa ? '完成!' : 'Done!'}</div>
        <div class="onb-title">${isJa ? 'あなたの最初の AI:' : 'Meet your first AI:'}</div>
      </div>
      <div class="onb-body">
        <div class="onb-agent">
          <div class="em">${esc(a.avatar || '🤖')}</div>
          <div class="nm">${esc(a.name || 'AI Agent')}</div>
          <div class="sk">${skills}</div>
          <div class="pe">${esc(a.persona || '').slice(0,240)}</div>
        </div>
        <button class="onb-cta" id="onbStartChat">${isJa ? '💬 最初のメッセージを送る →' : '💬 Send your first message →'}</button>
      </div>
    `;
    document.getElementById('onbStartChat').onclick = () => {
      _closeOnboarding();
      try {
        if(a && a.id){
          openAgent(a.id);
          setTimeout(() => {
            const inp = document.getElementById('ci') || document.getElementById('msgInput');
            if(inp){
              // Pre-fill the first message so sending it is one tap (Enter),
              // not a blank-box decision. Only ~22% of signups ever send a
              // first message — removing the "what do I type" friction here
              // is the highest-leverage activation fix.
              inp.value = isJa
                ? '「' + (state.goal || 'この目標') + '」を達成するための最初の一歩を、具体的に提案して。'
                : 'Suggest concrete first steps to achieve: ' + (state.goal || 'this goal');
              try { if(typeof exTA === 'function') exTA(inp); } catch(e){}
              inp.focus();
    ...
    card.innerHTML = `
      <div class="onb-hd"><div class="onb-step">Error</div><div class="onb-title">${isJa ? 'うまく行きませんでした' : 'Something went wrong'}</div></div>
      <div class="onb-body">
        <div class="onb-err">${esc(state.error || 'unknown')}</div>
        <button class="onb-cta" onclick="_renderOnboardingStep(1, {})">${isJa ? '← もう一度' : '← Try again'}</button>
        <button class="onb-skip" onclick="_skipOnboarding()">${isJa ? '今はスキップ' : 'Skip for now'}</button>
      </div>
    `;
  }
}
async function _runOnboardingGen(goal){
  try {
    const r = await api('POST', '/api/onboarding/quickstart', { goal, lang: isJa ? 'ja' : 'en' });
    if(!r || !r.agent) throw new Error('no agent returned');
    // Update local state so the new agent shows in the sidebar.
    if(!Array.isArray(me.agents)) me.agents = [];
    me.agents.push(r.agent);
    if(Array.isArray(agents)) agents.push(r.agent);
    me.onboarded_at = new Date().toISOString();
    try { renderAgList(); } catch(e){}
    _renderOnboardingStep(3, { agent: r.agent, goal });
  } catch(e){
    _renderOnboardingStep('error', { error: e.message || 'unknown' });
  }
}
async function _skipOnboarding(){
  try { await api('POST', '/api/onboarding/dismiss', {}); } catch(e){}
  if(me) me.onboarded_at = new Date().toISOString();
  _closeOnboarding();
}
