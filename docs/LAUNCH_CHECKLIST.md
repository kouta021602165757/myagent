...
> **今日完了した分（参考）**
> - ✅ Anthropic Tier 1 アップグレード
> - ✅ Supabase スキーマ自動マイグレーション (DATABASE_URL 設定済 + pg ライブラリ追加)
> - ✅ Google Sheets API 連携実装 (read/write/append/clear/get_meta/create_spreadsheet/add_sheet/format)
> - ✅ Google Sheets API 有効化 (Cloud Console)
> - ✅ OAuth リダイレクト URI / spreadsheets スコープ追加
> - ✅ Chrome ツールパス SSE ストリーミング対応
> - ✅ オンボーディングバナー (初回ユーザー向け Sheets 連携誘導)
> - ✅ ブラウザツール → Google Workspace URL の自動 Sheets API 誘導
...
### 2. Anthropic: Tier 1 へアップグレード ✅ **完了**
...
Stripe サポートに「クリエイターAgent Storeで 10% プラットフォーム手数料を引いて creator に送金」
という運用を伝えると審査が早い。
...
### 6.5. Google Sheets 連携（実施済 + 参考）✅

エンドユーザー視点の連携手順は `/setup-google-sheets.html` (アプリ管理者用) と
`/mock-sheets-flow.html` (エンドユーザー視点モック) を参照。

**これまで完了した管理者作業**:
- ✅ Supabase スキーマ移行 (`google_oauth jsonb` 列追加 — DATABASE_URL 経由で自動)
- ✅ Google Cloud Console OAuth クライアントに redirect URI 追加
  `https://myaiagents.agency/api/google/sheets/callback`
- ✅ OAuth 同意画面に `https://www.googleapis.com/auth/spreadsheets` スコープ追加
- ✅ Google Sheets API 有効化 (https://console.developers.google.com/apis/library/sheets.googleapis.com)

**エンドユーザーが各自やる**:
- アプリ画面でエージェント編集 → 📊 Google スプレッドシート連携 → 「+ Google アカウントと接続」 → トグル ON → 保存
...
→ Agent Store の自分の出店に青い ✓ が表示。OG 画像 (Pattern E) にも反映。
