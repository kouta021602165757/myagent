...
**B案（サーバサイド Playwright + Anthropic Tool Use）** で進めることに決定。

理由：
- ユーザーに何もインストールさせず動く
- 拡張開発・Chrome Web Store 申請不要で着手可能
- スクリーンショットを AI に直接見せられる

A案（Chrome 拡張）は Phase 3 で「ログインセッション必要なパワーユーザー向け」として後回し。
