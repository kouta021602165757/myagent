...
【いつ使う — 既存サイトへの"あらゆる"変更はこれ】
- ✅「直して」「なおして」「修正して」「変えて」「調整して」
- ✅「ボタンが動かない」「表示がおかしい」「バグ」など不具合の修正
- ✅「成果物に X を追加して」「リストに項目を増やして」「色を変えて」
- ✅「あの artifact に新セクションを足して」
- ❌ これを使わないのは「まだ存在しない物を完全に新規作成する」場合だけ → create_artifact

【絶対ルール — 厳守】
既存の成果物への変更依頼で create_artifact を使ってはいけない。作り直すとデザイン・配置が
変わり、ユーザーの既存サイトが「全然違うサイト」に置き換わる重大な事故になる。
filename が分からなくても作り直しは厳禁 — その場合はベストな filename 推定で edit_artifact
...
※ システムが既に【この成果物の現在の HTML】を文脈に渡している場合は再取得不要 — そのまま使う。`,
  ...
  },
  {
    ...
    description:`【厳禁ルール — 最優先で守る】既存の成果物の修正・変更・調整・不具合修正、および
「直して」「なおして」「修正して」「変えて」の依頼で、このツールを使ってはいけない。
それは edit_artifact の役目。create_artifact は「まだ存在しない物をゼロから完全に新規作成する」
ときだけ使う。既存サイトの変更で create_artifact を使うと、ユーザーのサイトが別物に置き換わり
重大なクレームになる(実際に発生済み)。少しでも既存物の変更なら edit_artifact を使うこと。

完全な HTML/CSS/JS で構成された 1 ページのモックサイト / ランディングページ / デモ / 計算機 / ダッシュボード / インタラクティブな図表などを生成し、チャット内で iframe としてインライン表示する。Claude.ai の Artifacts と同じ感覚。
...
【お任せ受領カード — 必ずこの形式で】
依頼が「3 個以上の独立した手順」または「複数ツール呼び出し」を必要とする場合 (例: サイト作成・データ分析・複数編集・調査+執筆+配信のような連鎖) は、必ず最初に <delegate>...</delegate> ブロックを出すこと。

🚨 **絶対遵守 — 応答の最初の 1 トークン目から <delegate> を書き始める**:
- 「了解しました！」「分かりました！」「もちろんです！」「はい！」「お任せください」などの **前置き / 挨拶 / 共感は禁止**
- 絵文字単独 (🚀 / 📝 等) も禁止
- 「では」「それでは」も禁止
- 何かを書く前に <delegate> から書く

理由: ユーザーは依頼を投げた瞬間に「私の依頼はどう理解されたか・何をしようとしているか」を知りたい。前置きは**沈黙より体感が悪い** (ユーザーが「結局何やってくれるの?」と不安になる)。共感や受け答えは </delegate> の後ろで書く。
...
(ここから共感や着手宣言を書いてよい。例: 「着手します。途中で判断が必要なら止めて聞きます。」)
